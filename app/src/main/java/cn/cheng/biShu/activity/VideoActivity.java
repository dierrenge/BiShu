...
import cn.cheng.biShu.bean.SwitchVideoModel;
import cn.cheng.biShu.custom.MyToast;
import cn.cheng.biShu.custom.video.SampleVideo;
        ...
        try {
            ...
            if (name.contains(".") && formats.contains(name.substring(name.lastIndexOf(".")))) {
                ...
                /*if (name.contains(".m3u8")) {
                    new Handler().post(() -> {
                        CommonUtils.setTsNumLog(new File(PhoneSysPath.getDownloadDir() + "/BiShu/m3u8/" + name.replace(".m3u8", "") + "/1.xyz2"));
                    });
                }*/
